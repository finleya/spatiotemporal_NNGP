...
p <- 1/(1+exp(-(x%*%beta+w)))
y <- rbinom(n, size=rep(1, n), prob=p) 

##
ho <- sample.int(n, floor(0.1*n))
...
neighbor.indx <- mk_nn_index(coords.mod, n.neighbors = 15)

## Check out the ordering and neighbors, just for fun.
n.indx <- neighbor.indx$n.indx #see spNNGP manual for description of n.indx.

i <- nrow(coords.mod)
spheres3d(coords.mod[-i,], col="gray", radius=0.001)
spheres3d(coords.mod[i,,drop=FALSE], col="blue", radius=0.01)
spheres3d(coords.mod[n.indx[[i]],,drop=FALSE], col="red", radius=0.01)
...
write.table(y.mod, "y.mod", row.names=F, col.names=F, sep="\t")
write.table(x.mod, "X.mod", row.names=F, col.names=F, sep="\t")
write.table(w.mod, "w.mod", row.names=F, col.names=F, sep="\t")
write.table(coords.mod, "coords.mod", row.names=F, col.names=F, sep="\t")

write.table(y.ho, "y.ho", row.names=F, col.names=F, sep="\t")
write.table(x.ho, "X.ho", row.names=F, col.names=F, sep="\t")
write.table(w.ho, "w.ho", row.names=F, col.names=F, sep="\t")
...
ns.cor <- function(v){
    h <- v[1] #space, gamma
    u <- v[2] #time, alpha
    1/((alpha*u^2 + 1)^kappa)*exp(-gamma*h/((alpha*u^2 + 1)^(kappa/2)))
}
