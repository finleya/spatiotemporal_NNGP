...
double dist3(double &a1, double &a2, double &a3, double &b1, double &b2, double &b3){
  return(sqrt(pow(a1-b1,2)+pow(a2-b2,2)+pow(a3-b3,2)));
}
...
double nsCor(double &h, double &u, double &a, double &c, double &k, int &covModel){
  ...
  if(covModel == 4){
    ...
    return 1.0/pow(a*pow(abs(u), 2.0) + 1.0, static_cast<double>(k))*exp(-c*h/pow(a*pow(abs(u), 2.0) + 1.0, static_cast<double>(k)/2.0));

  }else{
    Rf_error("c++ Rf_error: cov.model is not correctly specified in nsCor call\n");
  }
}
...
double spCor(double &D, double &phi, double &nu, int &covModel, double *bk){
  ...
  if(covModel == 0){//exponential
    ...
    Rf_error("c++ Rf_error: cov.model is not correctly specified in spCor call\n");
  }
}
