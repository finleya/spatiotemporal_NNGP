...
int main(int argc, char **argv){
  ...
  double *y0 = new double[n0*nSamples]; zeros(y0, n0*nSamples);
...
#pragma omp parallel for private(threadID, j, k, l, u, h, info)
	...
	for(l = 0; l < m; l++){
      ...
      y0[s*n0+i] = sqrt(tauSq[s])*yZ[zYIndx] + ddot_(&p, &X0[i], &n0, &beta[s*p], &inc) + w0[s*n0+i];
  ...
  writeRMatrix(outFile+"-y0", y0, n0, nSamples);
